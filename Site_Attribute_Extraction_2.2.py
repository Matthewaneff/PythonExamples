...
def writeLog(*argv):
	AddMessage(' ')
	AddMessage("===================================================================")
	sVersionInfo = 'Site_Attribute_Extraction_2.2.py, v20180124'
			...
			with da.InsertCursor(self.feature, ["SHAPE@", "SITE_ID", "NORTHING_m", "EASTING_m", "ACRES"]) as inscurs:
				...
				with da.SearchCursor(args[0], ["SHAPE@", args[1]]) as cursor:
					for row in cursor:
						...
						inscurs.insertRow([row[0], row[1], row[0].centroid.X, row[0].centroid.Y, row[0].getArea("PLANAR", "ACRES")])
...
################################################################################################################################################
	...
	def extract_PLSS(self, UID_List, townshipFeature, townshipField, sectionFeature, sectionField):
		
		# Make Feature Layers
		lyr = management.MakeFeatureLayer(self.feature, "Layer")
		township = management.MakeFeatureLayer(townshipFeature, "Township")
		section = management.MakeFeatureLayer(sectionFeature, "Section")
		
		# Loop through UID List and extract Township and Section attributes
		for d in UID_List:
			
			plss = []
			
			management.SelectLayerByAttribute(lyr, "NEW_SELECTION", "SITE_ID  = '{}'".format(d))
			management.SelectLayerByLocation(township, "INTERSECT", lyr, '', "NEW_SELECTION")
			
			townships = []
			
			# Collect ObjectIDs from Township layer and add it to townships list
			with da.SearchCursor(township, "OBJECTID") as cursor:
				for row in cursor:
					townships.append(row[0])
			
			# Loop through township list and perform selections
			for t in townships:
				management.SelectLayerByAttribute(township, "NEW_SELECTION", "OBJECTID = {}".format(t))
				management.SelectLayerByLocation(section, "HAVE_THEIR_CENTER_IN", township, '', "NEW_SELECTION")
				management.SelectLayerByLocation(section, "INTERSECT", lyr, '', "SUBSET_SELECTION")
				
				plssDict = {}
				
				with da.SearchCursor(township, townshipField) as tCurs:
					for row in tCurs:
						plssDict['Township'] = row[0]
				
				sections = []
				
				with da.SearchCursor(section, sectionField) as sCurs:
					s = []
					for row in sCurs:
						s.append(row[0])
					
					plssDict['Section'] = ', '.join(sorted(s))
				
				plss.append('{}: {}'.format(plssDict['Township'], plssDict['Section']))
			
			with da.UpdateCursor(self.feature, ["SITE_ID", "PLSS"]) as cursor:
				for row in cursor:
					if row[0] == d:
...
if __name__ == '__main__':
	...
	arg21 = GetParameterAsText(20) # Hydrology Datasets

	if arg21 != '':
		mainFeature.extractHydroAttributes(arg21.split(';'))
	
	''' Collect and Process PLSS Datasets'''
	
	arg22 = GetParameterAsText(21) # Township Featureclass
	arg23 = GetParameterAsText(22) # Township ID Field
	arg24 = GetParameterAsText(23) # Section Featureclass
	arg25 = GetParameterAsText(24) # Section ID Field
	
	if arg22 != '' and arg23 != '' and arg24 != '' and arg25 != '':
		
		textFields = ['PLSS']
		
		mainFeature.addFields(textFields=textFields)
		mainFeature.extract_PLSS(UIDs, arg22, arg23, arg24, arg25)
	...
	arg26 = GetParameterAsText(25) # Elevation Raster Dataset
	arg27 = GetParameterAsText(26) # Slope Raster Dataset
	arg28 = GetParameterAsText(27) # Aspect Raster Dataset
	...
	if mainFeature.desc.shapeType == 'Polygon':
		if arg26 != '':
			...
			processSpatialAttributes.append([arg26, 'Elevation_Avg', 'Elevation_Min', 'Elevation_Max'])
		...
		if arg27 != '':
			...
			processSpatialAttributes.append([arg27, 'Slope_Avg', 'Slope_Min', 'Slope_Max'])
		...
		if arg26 != '':
			...
			processSpatialAttributes.append([arg26, 'Elevation'])
			
		if arg27 != '':
			numFields.append('Slope')
			processSpatialAttributes.append([arg27, 'Slope'])
	...
	if arg28 != '':
		mainFeature.extractAspectAttributes(UIDs, arg28, 'Aspect')
